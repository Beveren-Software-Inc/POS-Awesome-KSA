    ...
    <v-dialog v-model="cancel_dialog" max-width="330">
      ...
      <v-card>
        <v-card-title class="text-h5">
          <span class="text-h5 text-primary">{{ $t("Cancel Sale ?") }}</span>
        </v-card-title>
        <v-card-text>
              ...
              <v-row class="ma-0 pa-0">
                ...
                <v-col cols="4">
                  <v-text-field
                    ...
                    variant="outlined"
                    color="primary"
                    :label="$t('Rate Inc. Tax')"
                    bg-color="white"
                    hide-details
                    :prefix="currencySymbol(pos_profile.currency)"
                    :model-value="formatCurrency(set_price_rate_with_tax(item))"
                    disabled
                  ></v-text-field>
                </v-col>
                ...
                <v-col cols="4" v-if="item.has_batch_no == 1 || item.batch_no">
                  <v-text-field
                    ...
                    variant="outlined"
                    color="primary"
                    :label="$t('Batch No. Available QTY')"
                    bg-color="white"
                    hide-details
          ...
          <v-row no-gutters class="pa-1 pt-9 pr-1">
            ...
            >
              <v-text-field
                ...
                :label="$t('Additional Discount')"
                id="additional_disacount_f"
                @change="calc_tot($event)"
                ref="discount"
                variant="outlined"
                background-color="white"
                density="compact"
            ...
            >
              <v-text-field
                ...
                :label="$t('Add Percentage Discount')"
                id="additional_discount_percentage_f"
                @change="calc_tot($event)"
                ref="discount"
                variant="outlined"
                background-color="white"
...
export default {
  ...
  data() {
    return {
      ...
      discount_amount_offer_name: null,
  ...
  methods: {
    ...
    load_all_customers() {
      ...
      if (!vm.pos_profile) {
        console.warn("POS Profile not set yet. Cannot load customers.");
        return;
      }
    ...
    get_shipping_rule_names() {
      const vm = this;
      frappe.call({
        method: "posawesome.posawesome.api.posapp.get_shipping_rule_names",
        callback: function (r) {
          if (r.message) {
            vm.shipping_rules = r.message;
          } else {
    ...
    onShippingRuleFocus() {
      if (!this.shipping_rules.length) {
        this.get_shipping_rule_names();
      } else {
        console.log("✅ Shipping rules already loaded");
      }
    },
    remove_item(item) {
    ...
    set_price_rate_with_tax(item) {
      const vm = this;
      let tax_rate = 0.15;
      let final_rate = item.rate;
     
      return (final_rate * (1 + tax_rate)).toFixed(2);
    },
    ...
    get_available_qty(item) {
      const vm = this;
      let item_available_qty = 0;
      frappe.call({
        method: "posawesome.posawesome.api.posapp.get_stock_availability",
        args: {
        ...
        if (item.has_serial_no && item.to_set_serial_no) {
          if (cur_item.serial_no_selected.includes(item.to_set_serial_no)) {
            this.eventBus.emit("show_message", {
              text: this.$t('serial_already_added', [
    ...
    set_bundle_discount(bundle_discount) {
      let amount = this.calc_total(this.get_invoice_items());

      this.discount_amount = flt(this.discount_amount) + flt(bundle_discount);
      this.discount_percentage_amount = flt(
        ((flt(this.discount_amount) / flt(amount)) * 100).toFixed(2)
      );
      this.calc_tot("additional_disacount_f");
    },
    ...
    new_invoice(data = {}) {
      ...
      if (!data.name && !data.is_return) {
        ...
        this.invoice_doc = "";
        this.discount_amount = 0;
        this.discount_percentage_amount = 0;
        this.invoiceType = "Invoice";
        this.invoiceTypes = ["Invoice", "Order"];
      } else {
        if (data.is_return) {
        ...
        this.shipping_rule = data.shipping_rule;
        this.customer = data.customer;
        this.discount_amount = data.discount_amount;
        this.discount_percentage_amount = data.additional_discount_percentage;
        this.items.forEach((item) => {
          if (item.serial_no) {
            item.serial_no_selected = [];
            const serial_list = item.serial_no.split("\n");
    ...
    get_invoice_tax() {
      const tax_list = [];
      if (this.shipping_rule) {
        const shipping_rule_value = frappe.get_doc(
          "Shipping Rule",
          this.shipping_rule
        );
        const new_tax = {
          charge_type: "Actual",
          account_head: shipping_rule_value.account,
          cost_center: shipping_rule_value.cost_center,
          tax_amount: shipping_rule_value.shipping_amount,
          description: shipping_rule_value.name,
        };
        tax_list.push(new_tax);
      }
      return tax_list;
    },
    ...
    async show_payment() {
      ...
      if (!this.items.length) {
        this.eventBus.emit("show_message", {
          // text: __(`Select items to sell`),
          text: this.$t("Select items to sell"),
          color: "error",
        });
        return;
      }
    ...
    validate() {
      ...
      var vm = this;
      this.items.forEach((item) => {
        item_av_qty = this.get_available_qty(item);
        if (
      ...
      "Discount percentage for item '{0}' cannot be greater than {1}%",
        ...
        if (item.qty == 0) {
          vm.eventBus.emit("show_message", {
            text: this.$t("Quantity for item '{0}' cannot be Zero (0)", [
              item.item_name,
            ]),
            color: "error",
          });
        ...
        if (
          ...
          vm.eventBus.emit("show_message", {
            text: this.$t("Maximum discount for Item {0} is {1}%", [
              item.item_name,
              item.max_discount,
            ]),
            color: "error",
        ...
        if (item.has_serial_no) {
          if (
            ...
            vm.eventBus.emit("show_message", {
              text: this.$t(`Selected serial numbers of item {0} is incorrect`, [
                item.item_name,
              ]),
              color: "error",
            });
        ...
        if (this.pos_profile.posa_allow_user_to_edit_additional_discount) {
          ...
          if (clac_percentage > this.pos_profile.posa_max_discount_allowed) {
            vm.eventBus.emit("show_message", {
              text: this.$t("The discount should not be higher than {0}%", [
                this.pos_profile.posa_max_discount_allowed,
              ]),
              color: "error",
            });
        ...
        if (this.invoice_doc.is_return) {
          if (this.subtotal >= 0) {
            vm.eventBus.emit("show_message", {
              text: this.$t("Return Invoice Total Not Correct"),
              color: "error",
            });
            value = false;
            return value;
          ...
          if (Math.abs(this.subtotal) > Math.abs(this.return_doc.total)) {
            vm.eventBus.emit("show_message", {
              text: this.$t("Return Invoice Total should not be higher than {0}", [
          ...
          this.items.forEach((item) => {
            ...
            if (!return_item) {
              ...
              vm.eventBus.emit("show_message", {
                text: this.$t("The QTY of the item {0} cannot be greater than {1}", [
                  item.item_name,
                  return_item.qty,
                ]),
                color: "error",
    ...
    fetch_customer_details() {
      const vm = this;
      if (vm.customer && vm.customers) {
        const selectedCustomer = vm.customers.find(
          (c) => c.name === vm.customer
        );

        if (selectedCustomer) {
          vm.customer_info = { ...selectedCustomer };
          vm.custom_discount_level =
            selectedCustomer.custom_discount_level || null;
          vm.eventBus.emit("set_customer_info", vm.customer_info);

          vm.update_price_list();
        } else {
          console.warn(this.$t("Selected customer not found in cached list."));
        }
      }
    },
    ...
    get_price_list() {
      ...
      if (vm.customer_info && this.pos_profile) {
    ...
    calc_tot($event) {
      ...
      if (event.target.id === "additional_disacount_f") {
        if (this.discount_amount < 0) {
          this.discount_percentage_amount = 0;
        } else {
          this.discount_percentage_amount = (
            (flt(this.discount_amount) / flt(amount)) *
            100
          ).toFixed(2);
        }
      } else if (event.target.id === "additional_discount_percentage_f") {
        this.discount_amount = (
          (flt(this.discount_percentage_amount) * flt(amount)) /
          100
        ).toFixed(2);
      }
    },

    calc_total(items_array) {
      let sum = 0;
    ...
    applyNewOffer(offer) {
      ...
      if (offer.offer === "Loyalty Point") {
        this.eventBus.emit("show_message", {
          text: this.$t("Loyalty Point Offer Applied"),
          color: "success",
        });
      }
    ...
    ApplyOnTotal(offer) {
      ...
      if (
        ...
        this.discount_percentage_offer_name = offer.name;
        this.discount_amount_offer_name = null;
      }

      else if (
        offer.discount_type === "Discount Amount" &&
        (!this.discount_amount_offer_name ||
          this.discount_amount_offer_name == offer.name) &&
    ...
    RemoveOnTotal(offer) {
      if (
        ...
        (this.discount_amount_offer_name &&
          this.discount_amount_offer_name == offer.offer_name)
      ) {
        this.discount_amount = 0;
        this.discount_percentage_offer_name = null;
        this.discount_amount_offer_name = null;
    ...
    print_draft_invoice() {
      if (!this.pos_profile.posa_allow_print_draft_invoices) {
        this.eventBus.emit("show_message", {
          text: this.$t("You are not allowed to print draft invoices"),
          color: "error",
        });
        return;
      }
  ...
  mounted() {
    ...
    this.eventBus.on("new_invoice", (data) => {
      this.invoice_doc = "";
      this.cancel_invoice();
    });
    ...
    custom_discount_level: {
      handler(newVal) {
        if (this.invoice_doc && Array.isArray(this.invoice_doc.items)) {
          this.invoice_doc.items.forEach((item, idx) => {
            this.update_item_rate(item);
            this.$set(this.invoice_doc.items, idx, { ...item }); // ensure reactivity
          });
